...
const searchUsda = async (query, attempt = 1) => {
  const params = new URLSearchParams({ api_key: process.env.USDA_FDC_API_KEY, query, pageSize: '25', dataType: 'Branded,SR Legacy,Survey (FNDDS)' })
  ...
  if (!response.ok) {
    ...
    if (response.status !== 429 && attempt < 3) { await sleep(300 * attempt); return searchUsda(query, attempt + 1) }
...
export default async (request) => {
  ...
  try {
    ...
    const [first, synonymResult] = await Promise.all([searchUsda(trimmed), synonymQuery ? searchUsda(synonymQuery) : null])
    if (!first.ok) {
