...
function App() {
  ...
  const saveCustomFood = async () => {
    if (!session || !foodDraft.name.trim() || !foodDraft.serving.trim()) return
    setSaving(true); setMessage('')
    const image_url = ingredients.length === 1 ? ingredients[0].imageUrl : null
    ...
    setSaving(false)
    if (logged) { setMessage(`${data.name} saved and added to today.`); setFoodDraft(emptyFood()); setIngredients([]); setShowCustomFoodForm(false); loadSavedFoods(session.user.id) }
  }
  ...
  if (step === 4) return <main className="dashboard">
    <div className="dashboard-content">
      ...
      {dashboardTab === 'foods' && <><div className="tab-header label-heading"><div><span className="eyebrow">Made by you</span><h1>My foods</h1></div><button className="add-label-button" onClick={() => { setFoodDraft(emptyFood()); setIngredients([]); setSelectedSavedFoodId(''); setIngredientSearchQuery(''); setIngredientSearchResults([]); setIngredientSearchError(''); setShowCustomFoodForm(true) }}>＋</button></div>{savedFoods.some(food => food.source_type === 'custom') ? <section className="food-library custom-library">{savedFoods.filter(food => food.source_type === 'custom').map(food => <article key={food.id}><div className="food-placeholder">♨</div><div><h2>{food.name}</h2><p>{food.serving_size}</p><span>{food.calories} cal · {food.protein_grams}g protein · {food.carbs_grams}g carbs</span></div><button disabled={saving} onClick={() => addSavedFood(food)}>Add</button></article>)}</section> : <section className="empty-library"><img src="/calpal-mascot.png" alt="" /><h2>Create your own foods</h2><p>Save meals, recipes, snacks, and drinks you eat often, then add a serving to today anytime.</p><button onClick={() => { setFoodDraft(emptyFood()); setIngredients([]); setSelectedSavedFoodId(''); setIngredientSearchQuery(''); setIngredientSearchResults([]); setIngredientSearchError(''); setShowCustomFoodForm(true) }}>＋ Create a food</button></section>}</>}
