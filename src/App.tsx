...
type FoodDraft = { name: string; serving: string; calories: string; protein: string; fiber: string; carbs: string; extras: ExtraFoodNutrient[]; servings: string; baseServing: string; baseCalories: string; baseProtein: string; baseFiber: string; baseCarbs: string; baseExtras: ExtraFoodNutrient[] }
...
const emptyFood = (): FoodDraft => ({ name: '', serving: '', calories: '', protein: '', fiber: '', carbs: '', extras: [], servings: '1', baseServing: '', baseCalories: '', baseProtein: '', baseFiber: '', baseCarbs: '', baseExtras: [] })
...
function WeeklyCalorieBalance({ logs, goal }: { logs: NutritionLog[]; goal: number }) {
  const days = useMemo(() => lastSevenDays().map(day => {
    const log = logs.find(l => l.log_date === day.dateStr)
    return { ...day, balance: log ? log.calories - goal : null }
  }), [logs, goal])
  return <section className="chart-card calorie-balance-card">
    <div className="chart-title"><div><span className="eyebrow">Vs. your goal</span><h2>Calorie balance</h2></div></div>
    <div className="calorie-balance">{days.map(day => <div key={day.dateStr} className="calorie-balance-day"><small>{day.label}</small><strong className={day.balance == null ? 'no-data' : day.balance > 0 ? 'surplus' : 'deficit'}>{day.balance == null ? '--' : `${day.balance > 0 ? '+' : ''}${day.balance.toLocaleString()} cal`}</strong></div>)}</div>
  </section>
}
...
function WeightChart({ weightLogs }: { weightLogs: WeightLog[] }) {
  ...
  if (!sorted.length) return <p className="empty-chart">Record your weight to start tracking your trend.</p>
...
function App() {
  ...
  const selectLabelImage = async (file?: File) => {
    ...
    try {
      ...
      setFoodDraft(current => ({ ...current, name: result.product_name || current.name, serving: result.serving_size || current.serving, calories: values.calories || '', protein: values.protein || '', fiber: values.fiber || '', carbs: values.carbs || '', extras, servings: '1', baseServing: result.serving_size || current.serving, baseCalories: values.calories || '', baseProtein: values.protein || '', baseFiber: values.fiber || '', baseCarbs: values.carbs || '', baseExtras: extras }))
    } catch (error) {
  ...
  const applyServings = (value: string) => {
    ...
    setFoodDraft(current => {
      ...
      return { ...current, servings: value, calories: scaleInt(current.baseCalories), protein: scaleDecimal(current.baseProtein), fiber: scaleDecimal(current.baseFiber), carbs: scaleDecimal(current.baseCarbs), extras: current.baseExtras.map(extra => ({ ...extra, value: scaleDecimal(extra.value) })) }
    })
  }
  ...
  const saveCustomFood = async () => {
    ...
    setSaving(false)
    if (logged) { setMessage(`${data.name} saved and added to today.`); setFoodDraft(emptyFood()); setLabelImage(null); setLabelPreview(''); setShowCustomFoodForm(false); loadSavedFoods(session.user.id) }
  }
