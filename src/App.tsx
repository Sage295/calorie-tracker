...
const matchNutrientKey = (name: string): NutrientKey | null => {
  ...
  if (lower.includes('trans')) return 'trans_fat'
  if (lower.includes('fat')) return 'fat'
  if (lower.includes('sugar')) return 'sugar'
  if (lower.includes('cholesterol')) return 'cholesterol'
...
function WeeklyCalorieBalance({ logs, onSelectDay }: { logs: NutritionLog[]; onSelectDay: (dateStr: string) => void }) {
  ...
  return <section className="chart-card calorie-balance-card">
    ...
    <div className="calorie-balance">{days.map(day => <button type="button" key={day.dateStr} className="calorie-balance-day" onClick={() => onSelectDay(day.dateStr)}><small>{day.label}</small><strong className={day.calories == null ? 'no-data' : 'logged'}>{day.calories == null ? '--' : `${day.calories.toLocaleString()} cal`}</strong></button>)}</div>
  </section>
}
...
function App() {
  ...
  const [editingFoodImageUrl, setEditingFoodImageUrl] = useState<string | null>(null)
  ...
  const searchFoods = async () => {
    ...
    try {
      const response = await fetch('/api/search-food', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: trimmed }) })
  ...
  const selectLabelImage = async (file?: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/scan-label', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image }) })
  ...
  const logFoodEntry = async (food: { id: number | null; name: string; serving_size: string; calories: number; protein_grams: number; fiber_grams: number; carbs_grams: number; image_url: string | null; extra_nutrients?: ExtraFoodNutrient[] }) => {
  ...
  const openEditCustomFood = (food: SavedFood) => {
    setFoodDraft({ name: food.name, serving: food.serving_size, calories: String(food.calories), protein: String(food.protein_grams), fiber: String(food.fiber_grams), carbs: String(food.carbs_grams), extras: food.extra_nutrients })
    setIngredients([]); setSelectedSavedFoodId(''); setIngredientSearchQuery(''); setIngredientSearchResults([]); setIngredientSearchError('')
    setCustomFoodImage(null); setCustomFoodImagePreview(food.image_url || '')
    setEditingFoodId(food.id); setEditingFoodImageUrl(food.image_url)
    setShowCustomFoodForm(true)
  }
  const saveCustomFood = async () => {
    if (!session || !foodDraft.name.trim() || !foodDraft.serving.trim()) return
    setSaving(true); setMessage('')
    let image_url = ingredients.length === 1 ? ingredients[0].imageUrl : editingFoodImageUrl
    if (customFoodImage) {
    ...
    const payload = { name: foodDraft.name.trim(), serving_size: foodDraft.serving.trim(), calories: Number(foodDraft.calories || 0), protein_grams: Number(foodDraft.protein || 0), fiber_grams: Number(foodDraft.fiber || 0), carbs_grams: Number(foodDraft.carbs || 0), extra_nutrients: foodDraft.extras, image_url }
    if (editingFoodId) {
      const { error } = await supabase.from('saved_foods').update(payload).eq('id', editingFoodId)
      setSaving(false)
      if (error) { setMessage(error.message); return }
      setMessage(`${payload.name} updated.`); setFoodDraft(emptyFood()); setIngredients([]); setCustomFoodImage(null); setCustomFoodImagePreview(''); setEditingFoodId(null); setEditingFoodImageUrl(null); setShowCustomFoodForm(false); loadSavedFoods(session.user.id)
      return
    }
    const { data, error } = await supabase.from('saved_foods').insert({ user_id: session.user.id, ...payload, source_type: 'custom' }).select().single()
    if (error) { setSaving(false); setMessage(error.message); return }
    const logged = await logFoodEntry({ id: data.id, name: data.name, serving_size: data.serving_size, calories: data.calories, protein_grams: data.protein_grams, fiber_grams: data.fiber_grams, carbs_grams: data.carbs_grams, image_url: data.image_url, extra_nutrients: data.extra_nutrients })
    setSaving(false)
  ...
  const deleteSavedFood = async (food: SavedFood) => {
    if (!session) return
    setSaving(true); setMessage('')
    const { error } = await supabase.from('saved_foods').delete().eq('id', food.id)
    setSaving(false)
    if (error) { setMessage(error.message); return }
    setMessage(`${food.name} deleted.`); loadSavedFoods(session.user.id)
  }
  ...
  if (step === 4) return <main className="dashboard">
    <div className="dashboard-content">
      ...
      {dashboardTab === 'progress' && <><div className="tab-header"><div><span className="eyebrow">Your stats</span><h1>Progress</h1></div><select value={period} onChange={e => setPeriod(e.target.value as Period)}><option value="week">Weekly</option><option value="month">Monthly</option><option value="year">Yearly</option></select></div><ProgressChart logs={logs} period={period} />{period === 'week' && <WeeklyCalorieBalance logs={logs} onSelectDay={setSelectedDayDate} />}<section className="chart-card weight-card"><div className="chart-title"><div><span className="eyebrow">Your trend</span><h2>Weight</h2></div><button className="record-weight-button" onClick={() => { setWeightDraft(profile.weight); setShowWeightForm(true) }}>Record weight</button></div><WeightChart weightLogs={weightLogs} /></section><section className="progress-note"><h2>Keep showing up.</h2><p>Each logged day makes your trends more useful. Your chart combines protein, fiber, and carbs from every daily entry.</p></section></>}
      {dashboardTab === 'labels' && <><div className="tab-header label-heading"><div><span className="eyebrow">Your label library</span><h1>Saved labels</h1></div><button className="add-label-button" onClick={() => { setFoodDraft(emptyFood()); setLabelImage(null); setLabelPreview(''); setScanError(''); setShowFoodForm(true) }}>＋</button></div>{savedFoods.some(food => food.source_type === 'label') ? <section className="food-library">{savedFoods.filter(food => food.source_type === 'label').map(food => <article key={food.id}>{food.image_url ? <img src={food.image_url} alt={`${food.name} nutrition label`} /> : <div className="food-placeholder">▤</div>}<div><h2>{food.name}</h2><p>{food.serving_size}</p><span>{food.calories} cal · {food.protein_grams}g protein · {food.carbs_grams}g carbs</span></div><div className="food-card-actions"><button className="delete-food-button" disabled={saving} aria-label={`Delete ${food.name}`} onClick={() => deleteSavedFood(food)}>×</button><button disabled={saving} onClick={() => addSavedFood(food)}>Add</button></div></article>)}</section> : <section className="empty-library"><img src="/calpal-mascot.png" alt="" /><h2>No saved labels yet</h2><p>Photograph a nutrition label, customize the food and serving, then reuse it whenever you eat it.</p><button onClick={() => setShowFoodForm(true)}>＋ Scan your first label</button></section>}</>}
      {dashboardTab === 'foods' && <><div className="tab-header label-heading"><div><span className="eyebrow">Made by you</span><h1>My foods</h1></div><button className="add-label-button" onClick={() => { setFoodDraft(emptyFood()); setIngredients([]); setSelectedSavedFoodId(''); setIngredientSearchQuery(''); setIngredientSearchResults([]); setIngredientSearchError(''); setCustomFoodImage(null); setCustomFoodImagePreview(''); setEditingFoodId(null); setEditingFoodImageUrl(null); setShowCustomFoodForm(true) }}>＋</button></div>{savedFoods.some(food => food.source_type === 'custom') ? <section className="food-library custom-library">{savedFoods.filter(food => food.source_type === 'custom').map(food => <article key={food.id}>{food.image_url ? <img src={food.image_url} alt={`${food.name}`} /> : <div className="food-placeholder">♨</div>}<div><h2>{food.name}</h2><p>{food.serving_size}</p><span>{food.calories} cal · {food.protein_grams}g protein · {food.carbs_grams}g carbs</span></div><div className="food-card-actions"><button className="delete-food-button" disabled={saving} aria-label={`Delete ${food.name}`} onClick={() => deleteSavedFood(food)}>×</button><button className="edit-food-button" disabled={saving} onClick={() => openEditCustomFood(food)}>Edit</button><button disabled={saving} onClick={() => addSavedFood(food)}>Add</button></div></article>)}</section> : <section className="empty-library"><img src="/calpal-mascot.png" alt="" /><h2>Create your own foods</h2><p>Save meals, recipes, snacks, and drinks you eat often, then add a serving to today anytime.</p><button onClick={() => { setFoodDraft(emptyFood()); setIngredients([]); setSelectedSavedFoodId(''); setIngredientSearchQuery(''); setIngredientSearchResults([]); setIngredientSearchError(''); setCustomFoodImage(null); setCustomFoodImagePreview(''); setEditingFoodId(null); setEditingFoodImageUrl(null); setShowCustomFoodForm(true) }}>＋ Create a food</button></section>}</>}
    ...
    {selectedDayDate && <div className="modal-backdrop" onMouseDown={() => setSelectedDayDate(null)}><section className="log-modal day-detail-modal" onMouseDown={e => e.stopPropagation()}><button className="close-button" onClick={() => setSelectedDayDate(null)}>×</button><span className="eyebrow">Daily total</span><h2>{new Date(`${selectedDayDate}T00:00:00`).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</h2>{(() => {
      const dayLog = logs.find(log => log.log_date === selectedDayDate)
      if (!dayLog) return <p className="day-detail-empty">No food logged this day.</p>
      return <div className="day-detail-list">
        <div className="day-detail-row"><span>Calories</span><strong>{dayLog.calories.toLocaleString()} cal</strong></div>
