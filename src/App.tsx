...
function CalorieChart({ logs, period, onPeriodChange }: { logs: NutritionLog[]; period: Period; onPeriodChange: (period: Period) => void }) {
  ...
  return <section className="home-chart">
    <div className="home-chart-header"><div><span>Macro breakdown</span><strong>{period === 'week' ? 'Last 7 days' : period === 'month' ? 'Last 4 weeks' : 'Last 12 months'}</strong><div className="home-legend"><span className="fiber-dot" />Fiber <span className="protein-dot" />Protein <span className="carbs-dot" />Carbs</div></div><select value={period} onChange={event => onPeriodChange(event.target.value as Period)} aria-label="Macro chart period"><option value="week">Weekly</option><option value="month">Monthly</option><option value="year">Yearly</option></select></div>
